...
void z80_cpu_do_interrupt(CPUState *cs)
{
    ...
    if (do_irq) {
       /* irq_vector = z80_intc_get_pending_vector(env->intc_handle,
						(env->sr >> 4) & 0xf);*/
        if (irq_vector == -1) {
            return; /* masked */
	}
    ...
    if (env->eflags & DELAY_SLOT_MASK) {
        /* Branch instruction should be executed again before delay slot. */
	env->spc -= 2;
	/* Clear flags for exception/interrupt routine. */
        env->eflags &= ~DELAY_SLOT_MASK;
    }
...
static int itlb_replacement(CPUZ80State * env)
{
...
*/
static int find_tlb_entry(CPUZ80State * env, target_ulong address,
...
static void increment_urc(CPUZ80State * env)
{
    uint8_t urb, urc;
...
/* Copy and utlb entry into itlb
   Return entry
*/
static int copy_utlb_entry_itlb(CPUZ80State *env, int utlb)
{
    int itlb;
    ...
    if (ientry->v) {
        tlb_flush_page(CPU(z80_env_get_cpu(env)), ientry->vpn << 10);
    }
...
*/
static int find_itlb_entry(CPUZ80State * env, target_ulong address,
                           int use_asid)
{
    int e;
...
static int find_utlb_entry(CPUZ80State * env, target_ulong address, int use_asid)
{
    /* per utlb access */
    increment_urc(env);
...
*/
static int get_mmu_address(CPUZ80State * env, target_ulong * physical,
...
static int get_physical_address(CPUZ80State * env, target_ulong * physical,
                                int *prot, target_ulong address,
                                int rw, int access_type)
{
 ...
 void cpu_z80_invalidate_tlb(CPUZ80State *s)
{
    ...
    tlb_flush(CPU(z80_env_get_cpu(s)));
}
				    ...
				    uint32_t mem_value)
{
    ...
    if (entry->v) {
        ...
        target_ulong address = entry->vpn << 10;
        tlb_flush_page(CPU(z80_env_get_cpu(s)), address);
    }
    entry->asid = asid;
    entry->vpn = vpn;
    entry->v = v;
				    ...
				    uint32_t mem_value)
{
	...
	for (i = 0; i < UTLB_SIZE; i++) {
	        ...
	        if (utlb_match_entry) {
                    CPUState *cs = CPU(z80_env_get_cpu(s));
	...
	for (i = 0; i < ITLB_SIZE; i++) {
        ...
        if (needs_tlb_flush) {
            tlb_flush_page(CPU(z80_env_get_cpu(s)), vpn << 10);
        }
...
int cpu_z80_is_cached(CPUZ80State * env, target_ulong addr)
{
     ...
     *        Now CCR is not in CPUZ80State, but in SH7750State.
     *        When you move the ccr into CPUZ80State, the code will be
     *        as follows.
     */
#if 0
